  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 dark:from-gray-900 dark:to-gray-800">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
        ...
        {/* Quick Stats - FlashScore Compact */}
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="grid grid-cols-3 gap-2 md:gap-4"
        >
          <div className="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col items-center gap-1 text-center">
            <Zap className="w-4 h-4 text-blue-600" />
            <p className="text-xs text-gray-600 dark:text-gray-400">Live</p>
            <p className="text-lg font-bold text-gray-900 dark:text-white">{totalLive}</p>
          </div>
          
          <div className="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col items-center gap-1 text-center">
            <TrendingUp className="w-4 h-4 text-purple-600" />
            <p className="text-xs text-gray-600 dark:text-gray-400">Leagues</p>
            <p className="text-lg font-bold text-gray-900 dark:text-white">{competitions.length}</p>
          </div>
          
          <div className="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col items-center gap-1 text-center">
            <AlertCircle className="w-4 h-4 text-green-600" />
            <p className="text-xs text-gray-600 dark:text-gray-400">Accuracy</p>
            <p className="text-lg font-bold text-gray-900 dark:text-white">90.3%</p>
          </div>
        </motion.div>
        ...
        {liveMatches.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Live</h2>
              <span className="bg-red-500 text-white px-2 py-0.5 rounded text-xs font-bold">{liveMatches.length}</span>
            </div>
              ...
              {liveMatches.map((match, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 rounded-lg p-2 border border-red-200 dark:border-red-800 hover:shadow-lg transition"
                  >
                    <div className="mb-2">
                      <p className="font-semibold text-gray-900 dark:text-white text-center text-xs leading-tight">{match.homeTeam}</p>
                      <div className="flex items-center justify-between gap-1 my-1">
                        <div className="flex-1"></div>
                        <div className="text-center">
                          <div className="text-lg font-bold text-gray-900 dark:text-white">{match.homeScore}-{match.awayScore}</div>
                          <div className="text-xs text-red-600 dark:text-red-400 font-bold animate-pulse leading-none">LIVE</div>
                        </div>
                        <div className="flex-1"></div>
                      </div>
                      <p className="font-semibold text-gray-900 dark:text-white text-center text-xs leading-tight">{match.awayTeam}</p>
                    </div>
                  </motion.div>
                );
              })}
